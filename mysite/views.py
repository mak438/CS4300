...
def moreReviews(request):
    ...
    if city == 'Urbana-Baseline':
        ...
        f = ReviewFinder(city)
    return render_to_response('./moreReviews.html', {'city': city, 'reviews': f.find_more(str(request.GET.get('review_id')), str(request.GET.get('reviewtext')), 20,"true"==str(request.GET.get('this_business_checked'))), 'reviewtext': str(request.GET.get('reviewtext'))})
