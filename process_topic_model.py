...
for b in pandas.read_json('Business.json').itertuples():
    s["b=" + str(b.business_id)] = (b.name, [cat for cat in b.categories if cat != 'Restaurants'], b.stars)
...
for term, reviews in reviews_by_term.items():
    s["t=" + term] = (reviews, term_frequencies[term])
...
topic_assignments = {t.topic: (t.title, t.color) for t in pandas.read_table('topics.txt', sep='-', header=None, names=['topic', 'title', 'color']).itertuples()}
...
for topic, reviews in reviews_by_topic.items():
    s["c=" + str(topic)] = (sorted(reviews, key=itemgetter(1), reverse=True), topic_assignments[topic])
